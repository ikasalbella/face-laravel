            ...
            <div class="flex items-center justify-between bg-gray-100 p-4 rounded-lg">
                ...
                <div>
                    <button type="submit" name="toggle" value="1"
                        ...
                        {{ optional($lokasi)->is_active ? 'bg-green-600 text-white' : 'bg-red-400 text-white' }}">
                        {{ optional($lokasi)->is_active ? 'ON (Aktif)' : 'OFF (Nonaktif)' }}
                    </button>
                </div>
            </div>
        </form>
        ...
        <form action="{{ route('admin.lokasi.update') }}" method="POST" class="space-y-4">
            ...
            <div class="flex items-center mt-2">
                <input type="checkbox" name="is_active" id="is_active"
                    {{ old('is_active', $lokasi->is_active ?? false) ? 'checked' : '' }}>
                <label for="is_active" class="ml-2 text-sm text-gray-700">Aktifkan Lokasi</label>
            </div>
